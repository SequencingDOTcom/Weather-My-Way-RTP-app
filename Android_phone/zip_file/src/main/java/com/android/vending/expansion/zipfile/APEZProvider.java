...
public abstract class APEZProvider extends ContentProvider {	
	...
	private boolean initIfNecessary() {
	    if ( !mInit ) {
            ...
            if ( null != pi.metaData ) {
                mainFileVersion = 24; //pi.metaData.getInt("mainVersion", appVersionCode);
                patchFileVersion = 24; //pi.metaData.getInt("patchVersion", appVersionCode);
            ...
            try {
                if ( null == resourceFiles ) {
                    mAPKExtensionFile = APKExpansionSupport.getAPKExpansionZipFile(ctx, 24, 24);  //(ctx, mainFileVersion, patchFileVersion);
                } else {
