...
- (void)viewDidLayoutSubviews{
    ...
    // NSLog(@"LocationVC: viewDidLayoutSubviews");
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    // NSLog(@"LocationVC: will Disappear");
...
- (void)initializeAndAddVideoToView {
    // set up video
    NSLog(@"LocationVC: initialize video player with layer");
...
- (IBAction)continueButtonPressed:(id)sender {
    ...
    if ([_locationTextField.text length] != 0) {
        ...
        if (_cityIsValidated && [_locationTextField.text length] != 0 && _nowSelectedLocation) {
            ...
            // we need to validate porovided location
            if ([InternetConnection internetConnectionIsAvailable]) {
        ...
        // we have not entered city into textField, location text field is empty
        dispatch_async(kMainQueue, ^{
            [self.activityProgress hide:YES];
            _cityIsValidated = NO;
            AlertMessage *alert = [[AlertMessage alloc] init];
    ...
    if ([_locationTextField.text length] != 0) {
        ...
        [wundergroundHelper wundergroundAutoCompleteValidateProvidedCity:_locationTextField.text withResult:^(NSArray *locations) {
            ...
            if (locations != nil) { // no results were returned > invalid city name was enteres
                if ([locations count] == 0) {
                ...
                } else if ([locations count] == 1) {    // valid city name was provided > we can show it in UI and save into user defaults
                    ...
                    if ([locationID length] != 0 && [locationName length] != 0 && [[location objectForKey:@"type"] isEqualToString:@"city"]) {
                        ...
                        NSLog(@"city validation: result is not valid");
                        dispatch_async(kMainQueue, ^{
                            [self.activityProgress hide:YES];
                            _cityIsValidated = NO;
                            AlertMessage *alert = [[AlertMessage alloc] init];
                ...
                } else if ([locations count] > 1) { // valid city name was provided > several cities with such name are available > let's clarify user city via picker view
                ...
                NSLog(@"city validation: result is nil");
                dispatch_async(kMainQueue, ^{
                    [self.activityProgress hide:YES];
                    _cityIsValidated = NO;
                    AlertMessage *alert = [[AlertMessage alloc] init];
