...
#pragma mark -
#pragma mark Is user authorized

- (BOOL)userIsAuthorized {
    SQToken *token = [self loadUserToken];
...
- (void)userDidSignOut {
    [_userDefaults removeObjectForKey:KEY_FOR_TOKEN];
...
- (SQToken *)loadUserToken {
    NSLog(@"UserHelper: loadUserToken");
    NSData *tokenData = [_userDefaults valueForKey:KEY_FOR_TOKEN];
...
- (NSString *)loadKnownVideoFileName {
    ...
    return [_userDefaults valueForKey:KEY_FOR_KNOWN_VIDEO_FILE_NAME];
}
...
- (void)saveUserToken:(SQToken *)token {
    if (token.accessToken != nil) {
        if (token.refreshToken != nil) {
            NSLog(@"UserHelper: saveUserToken %@", token.accessToken);
            NSLog(@"UserHelper: saveRefreshToken %@", token.refreshToken);
            
            // token is valid > save it into userDefaults
            NSData *tokenData = [NSKeyedArchiver archivedDataWithRootObject:token];
            [_userDefaults setValue:tokenData forKey:KEY_FOR_TOKEN];
            
        } else {
            NSLog(@"UserHelper: token.refreshToken is nil! Can't save token");
        }
    } else {
        NSLog(@"UserHelper: token.accessToken is nil! Can't save token");
    }
}
...
- (BOOL)locationIsEmpty:(NSDictionary *)location {
    ...
    if (location != nil && [locationAllKeys count] > 0) {
